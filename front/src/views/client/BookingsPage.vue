            ...
            <div class="flex-1">
              ...
              <div class="space-y-2">
                <div class="flex items-center gap-2 text-slate-600">
                  ...
                  <span>Дата бронирования: <span class="font-semibold">{{ formatDate(booking.booking_date) }}</span></span>
                </div>

                <div v-if="booking.child_full_name" class="flex items-center gap-2 text-slate-600">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                  </svg>
                  <span>Ребенок: <span class="font-semibold">{{ booking.child_full_name }}</span>{{ booking.child_age ? `, ${booking.child_age} лет` : '' }}</span>
                </div>
              </div>
            </div>
    ...
    <Modal v-model="showCancelModal" title="Отмена бронирования">
      <div v-if="bookingToCancel">
        ...
        <div class="p-4 bg-slate-50 rounded-lg">
          ...
          <p class="text-slate-600">{{ formatDate(bookingToCancel.booking_date) }}</p>
        </div>
      </div>
